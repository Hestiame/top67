...
const PROVIDERS = {
  groq: {
    ...
    body: (sys, user, model) => ({ model, temperature:0.15, max_tokens:8000,
...
function buildPrompt(opts){
  ...
  return `Você é um professor experiente montando um quiz para uso em SALA DE AULA.
A precisão factual é MAIS IMPORTANTE que a criatividade. Um erro factual é inaceitável.
...
1. Use APENAS fatos consolidados, que aparecem em livros didáticos e enciclopédias.
2. Se você não tiver CERTEZA ABSOLUTA de um fato, NÃO faça a pergunta. Escolha outro assunto do mesmo tema.
3. PROIBIDO inventar: nomes de pessoas, obras, tratados, batalhas, leis, datas exatas de eventos obscuros, estatísticas específicas e citações.
4. Prefira o que é amplamente conhecido e ensinado. Evite curiosidades duvidosas, "fatos surpreendentes" e detalhes muito específicos.
5. Em datas, prefira anos e séculos consagrados (ex.: 1888, 1500, século XIX). Evite dia e mês exatos.
6. As alternativas erradas devem ser plausíveis, mas NÃO podem ser factualmente ambíguas.
7. Cada pergunta deve ter UMA resposta indiscutivelmente correta. Se houver margem de discussão, troque a pergunta.

VARIE os tipos entre estes 5:
- "quiz": múltipla escolha com EXATAMENTE 4 alternativas, apenas 1 correta.
- "truefalse": afirmação para julgar Verdadeiro ou Falso.
- "type": resposta curta digitada (uma palavra ou nome).
- "puzzle": ordenar de 3 a 5 itens (cronológica, crescente, etc.).
- "slider": resposta que é um NÚMERO (ano, quantidade, porcentagem...).

Cerca de metade "quiz" e o resto distribuído. Inclua pelo menos um "truefalse", um "type" e um "slider" quando couber.
...
Responda SOMENTE com um objeto JSON válido neste formato exato:
{
  ...
  "questions": [
    { "type":"quiz", "text":"pergunta?", "fonte":"Por que a resposta está certa, em 1 frase objetiva.", "answers":[
        {"t":"A","correct":false},{"t":"B","correct":true},{"t":"C","correct":false},{"t":"D","correct":false}] },
    { "type":"truefalse", "text":"afirmação.", "fonte":"justificativa em 1 frase", "answers":[{"t":"Verdadeiro","correct":true},{"t":"Falso","correct":false}] },
    { "type":"type", "text":"pergunta curta?", "fonte":"justificativa em 1 frase", "accepted":["resposta","variação sem acento"] },
    { "type":"puzzle", "text":"ordene ...", "fonte":"critério da ordem em 1 frase", "items":["primeiro","segundo","terceiro","quarto"] },
    { "type":"slider", "text":"em que ano ...?", "fonte":"justificativa em 1 frase", "min":1900, "max":2000, "correct":1945, "step":1, "unit":"" }
  ]
}

O campo "fonte" é OBRIGATÓRIO em toda pergunta: explique em uma frase por que a resposta está correta. Se não conseguir escrever essa justificativa com segurança, a pergunta não deve existir — troque por outra.

Outras regras:
- Em "type", inclua variações aceitáveis (com/sem acento).
- Em "puzzle", os itens já na ORDEM CORRETA (o app embaralha depois).
- Em "slider", "correct" entre "min" e "max".
...
function sanitize(data, opts){
  ...
  for (const raw of (Array.isArray(data.questions)?data.questions:[])){
    ...
    const q = { id:'g'+Math.random().toString(36).slice(2,9), type:t, text:str(raw.text), time:TIME[t]||20, points:1000, fonte:str(raw.fonte).slice(0,300) };
    if(!q.text) continue;
    if(t==='quiz'){
