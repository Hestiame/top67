...
function joinScreen(){
  ...
  AVATARS.forEach((a)=>{
    const b=el(`<button class="av-pick ${a.id===me.avatar?'sel':''}" title="${esc(a.name)}">${renderCharacter(a.id,{},58)}</button>`);
  ...
  ACC_SLOTS.forEach(slot=>{
    ...
    ACC[slot].items.forEach(id=>{
      const b=el(`<button class="acc-pick ${me.acc[slot]===id?'sel':''}" title="${esc(ACC_NAMES[id]||id)}">${renderAccIcon(slot,id,44)}</button>`);
...
/* ---------- sessão (para voltar se a conexão cair) ---------- */
const SKEY='quizhub:sessao';
function salvarSessao(pin,d){ try{ sessionStorage.setItem(SKEY, JSON.stringify({pin,playerId:d.playerId,token:d.token,name:d.name,avatar:d.avatar,acc:me.acc})); }catch(e){} }
function lerSessao(){ try{ return JSON.parse(sessionStorage.getItem(SKEY)||'null'); }catch(e){ return null; } }
function limparSessao(){ try{ sessionStorage.removeItem(SKEY); }catch(e){} }

let tentandoVoltar=false;
function tentarVoltar(){
  const s=lerSessao(); if(!s||!s.pin||!s.token) return false;
  tentandoVoltar=true;
  me.name=s.name; me.avatar=s.avatar; me.acc=s.acc||{};
  socket.emit('player:resume',{pin:s.pin,playerId:s.playerId,token:s.token});
  return true;
}

/* ---------- eventos do servidor ---------- */
socket.on('player:joined',d=>{me.name=d.name;me.avatar=d.avatar;salvarSessao(ultimoPin,d);waitScreen(d.quizTitle);});
socket.on('player:error',m=>{limparSessao();toast(m);setTimeout(joinScreen,300);});
socket.on('player:info',m=>toast(m));
socket.on('player:kicked',m=>{limparSessao();toast(m);setTimeout(joinScreen,600);});

socket.on('player:resumed',d=>{
  tentandoVoltar=false;
  me.name=d.name; me.avatar=d.avatar; me.acc=d.acc||{};
  toast('Você voltou pro jogo! 🎉');
  if(d.phase==='question' && d.q && !d.answered) renderQuestion({q:d.q,qi:d.qi,total:d.total});
  else if(d.phase==='question' && d.answered) opinionSent();
  else waitScreen(d.quizTitle);
});
socket.on('player:resume:fail',()=>{ tentandoVoltar=false; limparSessao(); joinScreen(); });

socket.on('connect',()=>{ if(lerSessao() && !tentandoVoltar) tentarVoltar(); });
...
socket.on('player:end',d=>{limparSessao();endScreen(d);});
socket.on('disconnect',()=>toast('Conexão caiu — tentando voltar...'));

if(!tentarVoltar()) joinScreen();
