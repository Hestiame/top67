...
/* ---------- barra de avisos do sistema ---------- */
function sysbar(msg, ruim){
  let b=document.getElementById('sysbar');
  if(!msg){ if(b) b.remove(); document.body.style.paddingTop=''; return; }
  if(!b){ b=document.createElement('div'); b.id='sysbar'; document.body.appendChild(b); }
  b.className='sysbar'+(ruim?' bad':''); b.textContent=msg;
  document.body.style.paddingTop='34px';
}
async function checarSistema(){
  try{
    const s=await (await fetch('/api/status')).json();
    if(!s.bancoOk) sysbar('⚠️ Banco de dados fora do ar — usando cópia local. Alterações podem não ser salvas.', true);
    else sysbar('');
  }catch(e){ sysbar('⚠️ Sem conexão com o servidor.', true); }
}
setInterval(checarSistema, 60000);
checarSistema();

/* ---------- respostas de API com erro amigável ---------- */
async function apiJson(url, opts){
  const r = await fetch(url, opts);
  if(r.status===401){ toast('Sua sessão expirou — entre de novo'); setTimeout(()=>location.href='/host.html',900); throw new Error('auth'); }
...
async function home(){
  ...
  grid.querySelectorAll('[data-del]').forEach(b=>b.onclick=async()=>{
    ...
    if(confirm('Apagar o quiz "'+q.title+'"?\nIsso não tem como desfazer.')){
      try{ await apiJson('/api/quiz/'+q.id,{method:'DELETE'}); toast('Quiz apagado'); home(); }catch(e){ if(e.message!=='auth') toast(e.message||'Erro ao apagar'); }
    }
  });
}
...
function lobby(){
  ...
  document.getElementById('pbody').innerHTML=`<div class="lobby">
    ...
    <span class="scorepill" id="pcount" style="margin-top:6px">0 jogadores</span>
    ${trancada?`<span class="chip" style="background:#FFD86B;color:#5A4300;margin-top:6px">🔒 Sala trancada</span>`:''}
    <div class="players-join" id="pj"></div></div>`;
  document.getElementById('pfoot').innerHTML='';
  footBtn(trancada?'🔓 Destrancar':'🔒 Trancar sala',()=>socket.emit('host:lock',{trancada:!trancada}),true);
  footBtn('Começar →',()=>socket.emit('host:start'));
}
function renderRoster(players){
  const pj=document.getElementById('pj');if(!pj)return;
  pj.innerHTML='';
  if(!players.length){ pj.innerHTML=`<span style="opacity:.7;font-weight:700">aguardando jogadores…</span>`; return; }
  players.forEach(p=>{
    const chip=el(`<span class="pj ${p.online===false?'off':''}" title="${p.online===false?'desconectado':''}">
      ${renderCharacter(p.avatar,p.acc,28)}${esc(p.name)}
      <button class="pj-x" title="Remover jogador">✕</button></span>`);
    chip.querySelector('.pj-x').onclick=(e)=>{
      e.stopPropagation();
      if(confirm('Remover "'+p.name+'" da sala?')) socket.emit('host:kick',{playerId:p.id});
    };
    pj.appendChild(chip);
  });
  const c=document.getElementById('pcount');
  if(c) c.textContent = players.length + (players.length===1?' jogador':' jogadores');
}

/* ---------- pergunta ---------- */
function startTick(time, endsAt){
  clearInterval(tick);
  const t=document.getElementById('timer');
  if(!t||time<=0)return;
  const fim = endsAt || (Date.now()+time*1000);   // relógio vem do servidor
  const pinta=()=>{
    const rem=Math.max(0,Math.ceil((fim-Date.now())/1000));
    t.textContent=rem; t.classList.toggle('low',rem<=5);
    if(rem<=0) clearInterval(tick);
  };
  pinta();
  tick=setInterval(pinta,250);
}
...
function hostEnd(d){
  ...
  document.getElementById('pbody').innerHTML=`
    <div class="pod-wrap">
      <div class="pod-rays"></div>
      <h1 class="pod-title">Pódio</h1>
      <div class="podium">`+
      order.filter(i=>top[i]).map(i=>{const p=top[i],place=i+1;
        return `<div class="pod p${place}">
          <div class="pod-char">
            ...
            ${renderCharacter(p.avatar,p.acc,place===1?96:78)}
          </div>
          <div class="colp">
            <span class="rk">${place}º</span>
            <span class="nm">${esc(p.name)}</span>
            <span class="sc">${p.score} pts</span>
          </div></div>`;
      }).join('')+`</div>
      ${top.length>3?`<div class="pod-rest">`+top.slice(3,8).map(p=>`<span class="pod-chip">${renderCharacter(p.avatar,p.acc,22)} <b>${p.rank}º</b> ${esc(p.name)} · ${p.score}</span>`).join('')+`</div>`:''}
    </div>`;
  document.getElementById('pfoot').innerHTML='';
...
/* ---------- eventos ---------- */
const HKEY='quizhub:host';
function salvarHost(d){ try{ sessionStorage.setItem(HKEY, JSON.stringify({pin:d.pin,hostToken:d.hostToken})); }catch(e){} }
function lerHost(){ try{ return JSON.parse(sessionStorage.getItem(HKEY)||'null'); }catch(e){ return null; } }
function limparHost(){ try{ sessionStorage.removeItem(HKEY); }catch(e){} }
let tentandoHost=false;
function tentarVoltarHost(){
  const s=lerHost(); if(!s||!s.pin||!s.hostToken) return false;
  tentandoHost=true; socket.emit('host:resume',{pin:s.pin,hostToken:s.hostToken}); return true;
}

socket.on('host:created',d=>{game=d;trancada=false;salvarHost(d);lobby();});
socket.on('host:locked',d=>{trancada=d.trancada;toast(trancada?'Sala trancada 🔒':'Sala destrancada 🔓');if(game&&!curQ)lobby();});
socket.on('host:resume:fail',()=>{tentandoHost=false;limparHost();home();});
socket.on('host:resumed',d=>{
  tentandoHost=false; game=d; trancada=!!d.trancada; salvarHost(d);
  toast('Você voltou pro jogo 🎉');
  if(d.phase==='lobby'){ lobby(); renderRoster(d.players||[]); }
  else if(d.phase==='question'&&d.question){ hostQuestion({index:d.index,total:d.total,question:d.question,endsAt:d.endsAt,shuffledItems:d.shuffledItems}); }
  else if(d.phase==='leaderboard'){ hostLeaderboard({leaderboard:d.leaderboard,last:d.index+1>=d.total}); }
  else { lobby(); renderRoster(d.players||[]); }
});
socket.on('connect',()=>{ if(lerHost()&&!tentandoHost&&!game) tentarVoltarHost(); });
...
socket.on('host:end',d=>{limparHost();hostEnd(d);});
...
function aiPreview(quiz, params){
  ...
  root.innerHTML=`<div class="hwrap">
    ...
    <div style="max-width:760px;margin:0 auto">
      ...
      <span class="chip muted">${quiz.questions.length} perguntas geradas</span>
      <div style="background:#FFF7E6;border:1px solid #FFD98A;border-radius:12px;padding:12px 14px;margin-top:12px">
        <b style="font-size:14px">⚠️ Confira antes de usar</b>
        <p style="font-size:13px;color:#7A5A12;font-weight:600;margin-top:4px">A IA pode errar. Cada pergunta traz a justificativa da resposta — leia e corrija o que estiver errado antes de aplicar em prova.</p>
      </div>
  ...
  quiz.questions.forEach((q,i)=>{
    qs.appendChild(el(`<div style="background:var(--card);border:1px solid var(--line);border-radius:14px;padding:16px;margin-bottom:12px;box-shadow:var(--shadow-sm)">
      ...
      ${previewDetail(q)}
      ${q.fonte?`<div style="margin-top:10px;padding:9px 11px;background:var(--bg);border-radius:10px;border-left:3px solid var(--brand2)"><span style="font-size:11px;font-weight:800;color:var(--muted);text-transform:uppercase">Por que está certo</span><p style="font-size:13px;font-weight:600;margin-top:2px">${esc(q.fonte)}</p></div>`:''}
      </div>`));
  });
...
async function saveEditor(thenPlay){
  ...
  try{
    await apiJson('/api/quiz',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(ed.quiz)});
  ...
  }catch(e){ if(e.message!=='auth') toast(e.message||'Erro ao salvar'); }
}
